...
add_meta_box('event_social',  __('Social Icons', 'bp-create-events'), array( $this, 'social_show' ), 'event', 'normal', 'default');
add_meta_box('event_myfile',  __('Image', 'bp-create-events'), array( $this, 'myfile_show' ), 'event', 'normal', 'default');	
}
...
$date1 = get_post_meta( $post->ID, 'start-date', true );
$date2 = get_post_meta( $post->ID, 'end-date', true );
		...
		<p>
			<label for="end-date"><?php echo __( 'end Date:', 'bp-simple-events' ); ?></label>
			<input type="text" size="25" id="end-date" name="end-date" value="<?php echo $date2; ?>" />
		</p>
		<p>
...
function myfile_show( $post ) {

		wp_nonce_field( 'myfile_box', 'myfile_box_nonce' );

		$date = get_post_meta( $post->ID, 'event-myfile', true );

		$myfile = ! empty( $date ) ? $date : '';
		?>

		<p>
			<label for="event-myfile"><?php echo __( 'Event myfile:', 'bp-simple-events' ); ?></label>
			<input type="text" size="70" id="event-myfile" name="event-myfile" value="<?php echo $myfile; ?>" />
		</p>

		<?php
	}
function social_show( $post ) {

		wp_nonce_field( 'social_box', 'social_box_nonce' );

		$fb = get_post_meta( $post->ID, 'event-facebook', true );
		$tw = get_post_meta( $post->ID, 'event-twitter', true );
		$gg = get_post_meta( $post->ID, 'event-google', true );

		$facebook = ! empty( $fb ) ? $fb : '';
$twitter = ! empty( $tw ) ? $tw : '';
$google = ! empty( $gg ) ? $gg : '';
		?>

		<p>
			<label for="event-facebook"><?php echo __( 'Event facebook:', 'bp-create-events' ); ?></label>
			<input type="text" size="70" id="event-facebook" name="event-facebook" value="<?php echo $facebook; ?>" />
		</p>
		
		<p>
			<label for="event-twitter"><?php echo __( 'Event twitter:', 'bp-create-events' ); ?></label>
			<input type="text" size="70" id="event-twitter" name="event-twitter" value="<?php echo $twitter; ?>" />
		</p>
		<p>
			<label for="event-google"><?php echo __( 'Event google:', 'bp-create-events' ); ?></label>
			<input type="text" size="70" id="event-google" name="event-google" value="<?php echo $google; ?>" />
		</p>
...
$this->save_social( $post_id );

$this->save_myfile( $post_id );
...
private function save_myfile( $post_id ) {

		if ( ! isset( $_POST['myfile_box_nonce'] ) )
			return $post_id;

		$nonce = $_POST['myfile_box_nonce'];

		if ( ! wp_verify_nonce( $nonce, 'myfile_box' ) )
			return $post_id;

		$myfile = sanitize_text_field( $_POST['event-myfile'] );
		update_post_meta( $post_id, 'event-myfile', $myfile );

	}
private function save_social( $post_id ) {

		if ( ! isset( $_POST['social_box_nonce'] ) )
			return $post_id;

		$nonce = $_POST['social_box_nonce'];

		if ( ! wp_verify_nonce( $nonce, 'social_box' ) )
			return $post_id;

		$facebook = sanitize_text_field( $_POST['event-facebook'] );
		update_post_meta( $post_id, 'event-facebook', $facebook );
		$twitter = sanitize_text_field( $_POST['event-twitter'] );
		update_post_meta( $post_id, 'event-twitter', $twitter );
$google = sanitize_text_field( $_POST['event-google'] );
		update_post_meta( $post_id, 'event-google', $google );
	}
